...
Скрипт `agent.py` демонстрирует использование модели gemini-2.0-flash для построения плана и выполнения команд в рабочей директории. Перед запуском установите зависимости `pip install -r requirements.txt` и задайте переменную окружения `GEMINI`.
