...
Скрипт `agent.py` демонстрирует использование модели gemini-2.0-flash для построения плана и выполнения команд в рабочей директории. Перед запуском необходимо установить зависимости `pip install -r requirements.txt` и задать переменную окружения `GEMINI_API_KEY`.
